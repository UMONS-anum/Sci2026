...
def continuous_least_squares(x: np.ndarray, y: np.ndarray, breakpoints: list[int]):
    ''' Utilise sp.linalg.lstsq pour calculer les coefficients beta de la matrice B
    ...
    Return:
        ...
        M : la matrice M
        x_valid : les années pour lesquelles on a une date de floraison connue
        y_valid : les dates de floraisons non NaN
    '''
    ...
    beta = sp.linalg.lstsq(M, y_valid)[0] # = vecteur B des coefficients
...
def continuous_solve(x: np.ndarray, y: np.ndarray, breakpoints: list[int]):
    ...
    MT_M = MT @ M
    MT_y = MT @ y_valid
    beta = np.linalg.solve(MT_M, MT_y) # = vecteur B des coefficients
...
def evaluate_model(x: np.ndarray, beta: np.ndarray, breakpoints: list[int]) -> np.ndarray:
    ...
    Returns:
        np.ndarray : les valeurs du modèle aux points x
    '''
